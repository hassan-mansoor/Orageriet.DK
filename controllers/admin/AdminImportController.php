	...
	*/
	...
	public function categoryImport()
	{
		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++)
		{
			...
			if (isset($info['id']) && in_array($info['id'], $tab_categ))
			{
			...
			if (Tools::getValue('forceIDs') && isset($info['id']) && $info['id'])
			{ 

				$category = new Category($info['id']);
				
			}
			else
			{
				if (isset($info['id']) && $info['id'] && Category::existsInDatabase($info['id'], 'category'))
					$category = new Category($info['id']);
				else
				...
				($lang_field_error = $category->validateFieldsLang(UNFRIENDLY_ERROR, true)) === true && empty($this->errors))
			{
				...
				{
					$cat_moved[$category->id] = $category_already_created['id_category'];
					$category->id =	$category_already_created['id_category'];
				}
