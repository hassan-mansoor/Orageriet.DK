...
class AdminCategoriesControllerCore extends AdminController
{
	...
	public function init()
	{
		...
		$this->original_filter = $this->_filter .= ' AND `id_parent` = '.$id_parent.' ';
		...
		{
			$this->redirect_after = self::$currentIndex.'&id_category='.$this->context->shop->getCategory().'&token='.$this->token;
			$this->redirect();
		}
	}
	...
	public function getList($id_lang, $order_by = null, $order_way = null, $start = 0, $limit = null, $id_lang_shop = false)
	{
		...
		{
			$item = &$this->_list[$i];
			$category_tree = Category::getChildren($item['id_category'], $this->context->language->id);
	...
	public function initToolbar()
	{
		...
		if (count(Category::getCategoriesWithoutParent()) == 1 && !Tools::isSubmit('id_category')
			...
			$this->toolbar_btn['edit'] = array(
				'href' => self::$currentIndex.'&amp;update'.$this->table.'&amp;id_category='.$this->_category->id.'&amp;token='.$this->token,
				'desc' => $this->l('Edit')
			);
		...
		if (empty($this->display))
		{
			$id_category = (Tools::isSubmit('id_category')) ? '&amp;id_parent='.Tools::getValue('id_category') : '';
			$this->toolbar_btn['new'] = array(
	...
	public function renderForm()
	{
		...
		$selected_categories = array((isset($obj->id_parent) && $obj->isParentCategoryAvailable($id_shop))? $obj->id_parent : Tools::getValue('id_parent', Category::getRootCategory()->id));
		...
		$image = ImageManager::thumbnail(_PS_CAT_IMG_DIR_.'/'.$obj->id.'.jpg', $this->table.'_'.$obj->id.'.'.$this->imageType, 350, $this->imageType, true);
	...
	public function processAdd()
	{
		...
		if (!$id_parent)
		{
		   ...
		   $_POST['id_parent'] = $id_parent = Configuration::get('PS_ROOT_CATEGORY');
		}
		...
		if ($this->tabAccess['delete'] === '1')
		{
			...
			if (parent::processBulkDelete())
			{
					...
					foreach ($cats_ids as $id => $id_parent)
						$this->processFatherlessProducts($id_parent);
					return true;
			}
			else
				return false;
	...
	public function processDelete()
	{
		...
		{ 
			 ...
			 {
				 $this->setDeleteMode();
				 $this->processFatherlessProducts($category->id_parent);
	...
	public function processFatherlessProducts($id_parent)
	{
		...
		{
			$poor_product = new Product($id_poor_product['id_product']);
			if (Validate::isLoadedObject($poor_product))
			{
				...
				else
				{
					...
					$poor_product->id_category_default = $id_parent;
	...
	public function processPosition()
	{
		if ($this->tabAccess['edit'] !== '1')
			$this->errors[] = Tools::displayError('You do not have permission to edit this.');
		else if (!Validate::isLoadedObject($object = new Category(Tools::getValue($this->identifier, Tools::getValue('id_category_to_move', 1)))))
		...
		else
		{
			...
			Tools::redirectAdmin(self::$currentIndex.'&'.$this->table.'Orderby=position&'.$this->table.'Orderway=asc&conf=5'.(($id_category = Tools::getValue($this->identifier, Tools::getValue('id_category_parent', 1))) ? ('&'.$this->identifier.'='.$id_category) : '').'&token='.Tools::getAdminTokenLite('AdminCategories'));
		}
	}
	...
	{
		$id_category_to_move = (Tools::getValue('id_category_to_move'));
		$id_category_parent = (Tools::getValue('id_category_parent'));
