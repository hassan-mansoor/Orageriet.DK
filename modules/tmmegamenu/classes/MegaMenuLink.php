...
class MegaMenuLink extends ObjectModel
{
	...
	public function updateLink()
	{
		...
		if (!Tools::getValue('id_item')) // if item not exist create new one
		{
			...
			if (!Db::getInstance()->insert('tmmegamenu_link', $data) || !$id_item = Db::getInstance()->Insert_ID())
				return false;
			// add multilanguage fields to tmmegamenu_link_lang table
			foreach ($languages as $language)
			{
				// item title is required, add default langage title to all language if missed title field
				if (!Tools::isEmpty(trim(Tools::getValue('title_'.$language['id_lang']))))
				{
	...
	*****/
	public function deleteLink()
	{
		...
		if (!Db::getInstance()->delete('tmmegamenu_link', '`id_item` ='.$id_item.' AND `id_shop` = '.$id_shop)
			|| !Db::getInstance()->delete('tmmegamenu_link_lang', '`id_item` ='.$id_item))
