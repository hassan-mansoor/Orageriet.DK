...
class Tmmegamenu extends Module
{
	...
	public function getContent()
	{
		...
		{
			if (!$check_item_fields && !$check_html_fields && !$check_link_fields && !$check_banner_fields)
	...
	*****/
			...
			switch (Tools::substr($item, 0, Tools::strlen($values[1])))
			{
				...
				case 'HTML':
					...
					if (count($new_html = $new_html->getHtml((int)$id)) && $new_html)
						$html .= '<option selected="selected" value="HTML'.(int)$new_html['id_item'].'">'
									.Tools::safeOutput($new_html['title_'.$id_lang.'']).
								'</option>';
					break;
				case 'LNK':
	...
	protected function makeMenuTop()
	{
		...
		{
			$this->menu = '<ul class="menu clearfix">';
				foreach ($top_items as $key => $top)
				{
					...
					$this->menu .= '<li class="'.$top['specific_class'].$simple_class.$top_item_url['selected'].'">';
						if (!Tools::isEmpty($top_item_url['url']))
							$this->menu .= '<a href="'.$top_item_url['url'].'">'.$top['title'].$badge.'</a>';
						else
							$this->menu .= $top['title'];
					...
					if (!$top['is_mega'])
					{
						...
						if ($subitems)
						{
							$this->menu .= '<ul class="is-simplemenu">';
					...
					else
					{
						...
						{
							$this->menu .= '<div class="is-megamenu">';
								foreach ($rows as $row)
								{
	...
	protected function generateCustomHtml($id_item)
	{
		...
		if ($html)
			$output .= '<li '.($html['specific_class']?'class="'.$html['specific_class'].' html"':'class="html"').'>
							<h3>'.$html['title_'.(int)$this->context->language->id.''].'</h3>'
							.$html['content_'.(int)$this->context->language->id.''].
						'</li>';
		return $output;
	}
	...
	*****/
	protected function generateCustomLink($id_item)
	{
		...
		if ($html)
			$output .= '<li '.($html['specific_class']?'class="'.$html['specific_class'].' custom-link"':'class="custom-link"').'>
							<a '.($html['blank']?'target="_blank"':'').' href="'.$html['url_'.(int)$this->context->language->id.''].'">'
								.$html['title_'.(int)$this->context->language->id.''].
							'</a>
						</li>';
		return $output;
	}
	...
	*****/
	...
	protected function getWarningMultishop()
	{
		if (Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_ALL)
			return $this->displayWarning($this->l('You cannot manage slides
