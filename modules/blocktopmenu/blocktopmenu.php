...
class Blocktopmenu extends Module
{
    ...
    protected function makeMenuOption()
    {
        ...
        foreach ($menu_item as $item) {
            ...
            $id = substr($item, strlen($values[1]), strlen($item));

            switch (substr($item, 0, strlen($values[1]))) {
                case 'CAT':
                    $category = new Category($id, (int)$id_lang);
    ...
    protected function makeMenu()
    {
        ...
        foreach ($menu_items as $item) {
            ...
            $id = substr($item, strlen($value[1]), strlen($item));
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'SHOP':
                    ...
                    break;
                case 'LNK':
                    $link = MenuTopLinks::get($id, (int)$id_lang, (int)$id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
    ...
    protected function generateCategoriesOption($categories, $items_to_skip = null)
    {
        ...
        foreach ($categories as $key => $category) {
            if (isset($items_to_skip) /*&& !in_array('CAT'.(int)$category['id_category'], $items_to_skip)*/) {
                ...
                $html .= '<option value="CAT'.$category['id_category'].'">'
    ...
    protected function generateCategoriesMenu($categories, $is_children = 0)
    {
        ...
        foreach ($categories as $key => $category) {
            ...
            $html .= '<li'.(($this->page_name == 'category'
                && Tools::getValue('id_category') == $category['id_category']) ? ' class="sfHoverForce"' : '').'>';
