...
class AdminControllerCore extends Controller
{
	...
	public function processAdd()
	{
		...
		if (count($this->errors) <= 0)
		{
			...
			{	
				PrestaShopLogger::addLog(sprintf($this->l('%s addition', 'AdminTab', false, false), $this->className), 1, null, $this->className, $this->object->id, true, (int)$this->context->employee->id);
				$parent_id = Tools::getValue('id_parent', 1);
	...
	public function processStatus()
	{
		if (Validate::isLoadedObject($object = $this->loadObject()))
		{
			if ($object->toggleStatus())
			{
				...
				$id_category = (($id_category = Tools::getValue('id_category')) && Tools::getValue('id_product')) ? '&id_category='.$id_category : '';
	...
	protected function updateAssoShop($id_object)
	{
		...
		Db::getInstance()->delete($this->table.'_shop', '`'.$this->identifier.'` = '.$id_object.($exclude_ids ? ' AND id_shop NOT IN ('.implode(', ', $exclude_ids).')' : ''));
		...
		if (is_array($this->boxes) && !empty($this->boxes))
		{
			...
			else
			{
				...
				foreach ($this->boxes as $id)
				{
					...
					if ($delete_ok)
						PrestaShopLogger::addLog(sprintf($this->l('%s deletion', 'AdminTab', false, false), $this->className), 1, null, $this->className, $to_delete->id, true, (int)$this->context->employee->id);
					else
