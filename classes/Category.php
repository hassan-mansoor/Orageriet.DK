...
class CategoryCore extends ObjectModel
{
	...
	public function __construct($id_category = null, $id_lang = null, $id_shop = null)
	{
		...
		$this->id_image = ($this->id && file_exists(_PS_CAT_IMG_DIR_.$this->id.'.jpg')) ? $this->id : false;
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		if ($this->is_root_category && ($id_root_category = Configuration::get('PS_ROOT_CATEGORY')))
		...
		if (Tools::isSubmit('checkBoxShopAsso_category'))
			...
			{
				$position = Category::getLastPosition($this->id_parent, $id_shop);
			...
			{
				$position = Category::getLastPosition($this->id_parent, $shop['id_shop']);
	...
	public function update($null_values = false)
	{
		...
		if ($this->is_root_category && $this->id_parent != Configuration::get('PS_ROOT_CATEGORY'))
		...
		if ($changed)
		{
			if (Tools::isSubmit('checkBoxShopAsso_category'))
				foreach (Tools::getValue('checkBoxShopAsso_category') as $id_asso_object => $row)
					...
					{
					$this->addPosition(Category::getLastPosition($this->id_parent, $id_shop), $id_shop);
					}
			else
				foreach (Shop::getShops(true) as $shop)
			
					$this->addPosition(Category::getLastPosition($this->id_parent, $shop['id_shop']), $shop['id_shop']);
		}
	...
	protected function recursiveDelete(&$to_delete, $id_category)
	{
		...
		{
			$to_delete[] = $row['id_category'];
			$this->recursiveDelete($to_delete, $row['id_category']);
		}
	}
	...
	public function delete()
	{
		...
		if ($this->id === 0 || $this->id === Configuration::get('PS_ROOT_CATEGORY'))
			return false;
		...
		foreach ($all_cat as $cat)
		{
			...
			if (!$this->hasMultishopEntries())
			{
				...
				if (GroupReduction::getGroupsReductionByCategoryId($cat->id))
	...
	public static function regenerateEntireNtree()
	{
		...
		ON (c.`id_category` = cs.`id_category` AND cs.`id_shop` = '.$id_shop.')
	...
	{
		$left = $n++;
		if (isset($categories[$id_category]['subcategories']))
			foreach ($categories[$id_category]['subcategories'] as $id_subcategory)
				Category::_subTree($categories, $id_subcategory, $n);
		$right = (int)$n++;
												...
												$use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '')
	{
		...
		$cache_id = 'Category::getAllCategoriesName_'.md5($root_category.(int)$id_lang.(int)$active.(int)$use_shop_restriction
		...
		if (!Cache::isStored($cache_id))
		{
			$result = Db::getInstance()->executeS('
				...
				'.(isset($root_category) ? 'RIGHT JOIN `'._DB_PREFIX_.'category` c2 ON c2.`id_category` = '.$root_category.' AND c.`nleft` >= c2.`nleft` AND c.`nright` <= c2.`nright`' : '').'
				WHERE 1 '.$sql_filter.' '.($id_lang ? 'AND `id_lang` = '.(int)$id_lang : '').'
		...
		$use_shop_restriction = true, $sql_filter = '', $sql_sort = '', $sql_limit = '')
	{
		...
		$cache_id = 'Category::getNestedCategories_'.md5($root_category.(int)$id_lang.(int)$active.(int)$use_shop_restriction
			.(isset($groups) && Group::isFeatureActive() ? implode('', $groups) : ''));
	...
	public function getSubCategories($id_lang, $active = true)
	{
		...
		foreach ($result as &$row)
		{
			$row['id_image'] = Tools::file_exists_cache(_PS_CAT_IMG_DIR_.$row['id_category'].'.jpg') ? $row['id_category'] : Language::getIsoById($id_lang).'-default';
	...
	public function getProducts($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
	{
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) AS quantity'.(Combination::isFeatureActive() ? ', IFNULL(product_attribute_shop.id_product_attribute, 0) AS id_product_attribute,
				...
				ON (p.`id_product` = product_attribute_shop.`id_product` AND product_attribute_shop.`default_on` = 1 AND product_attribute_shop.id_shop='.$context->shop->id.')':'').'
				...
				LEFT JOIN `'._DB_PREFIX_.'image_shop` image_shop
					ON (image_shop.`id_product` = p.`id_product` AND image_shop.cover=1 AND image_shop.id_shop='.$context->shop->id.')
				LEFT JOIN `'._DB_PREFIX_.'image_lang` il
					ON (image_shop.`id_image` = il.`id_image`
					AND il.`id_lang` = '.(int)$id_lang.')
				LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
					ON m.`id_manufacturer` = p.`id_manufacturer`
				WHERE product_shop.`id_shop` = '.$context->shop->id.'
					AND cp.`id_category` = '.$this->id
					.($active ? ' AND product_shop.`active` = 1' : '')
					.($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '')
					.($id_supplier ? ' AND p.id_supplier = '.(int)$id_supplier : '')
	...
	public static function getChildren($id_parent, $id_lang, $active = true, $id_shop = false)
	{
		...
		$cache_id = 'Category::getChildren_'.$id_parent.'-'.(int)$id_lang.'-'.(bool)$active.'-'.$id_shop;
		if (!Cache::isStored($cache_id))
		{
	...
	public static function hasChildren($id_parent, $id_lang, $active = true, $id_shop = false)
	{
		...
		$cache_id = 'Category::hasChildren_'.$id_parent.'-'.(int)$id_lang.'-'.(bool)$active.'-'.$id_shop;
		if (!Cache::isStored($cache_id))
		{
	...
	public static function duplicateProductCategories($id_old, $id_new)
	{
		$sql = 'SELECT `id_category`
				...
				WHERE `id_product` = '.$id_old;
		$result = Db::getInstance()->executeS($sql);

		$row = array();
		if ($result)
			foreach ($result as $i)
				$row[] = '('.implode(', ', array($id_new, $i['id_category'], '(SELECT tmp.max + 1 FROM (
					SELECT MAX(cp.`position`) AS max
					FROM `'._DB_PREFIX_.'category_product` cp
					WHERE cp.`id_category`='.$i['id_category'].') AS tmp)'
				)).')';
	...
	public static function checkBeforeMove($id_category, $id_parent)
	{
		...
		while (42)
		{
			$result = Db::getInstance()->getRow('SELECT `id_parent` FROM `'._DB_PREFIX_.'category` WHERE `id_category` = '.$i);
	...
	public static function getLinkRewrite($id_category, $id_lang)
	{
		...
		if (!isset(self::$_links[$id_category.'-'.$id_lang]))
			...
			AND cl.`id_category` = '.$id_category);
	...
	public static function searchByName($id_lang, $query, $unrestricted = false, $skip_cache = false)
	{
		if ($unrestricted === true)
		{
			...
			AND c.`id_category` != '.Configuration::get('PS_HOME_CATEGORY'));
	}
	...
	public static function searchByNameAndParentCategoryId($id_lang, $category_name, $id_parent_category)
	{
		...
		LEFT JOIN `'._DB_PREFIX_.'category_lang` cl
			...
			AND `id_lang` = '.$id_lang.Shop::addSqlRestrictionOnLang('cl').')
		WHERE `name` = \''.pSQL($category_name).'\'
			AND c.`id_category` != '.Configuration::get('PS_HOME_CATEGORY').'
			AND c.`id_parent` = '.$id_parent_category);
	}
...
* 	 * @param bool $method_to_create a category
		...
		if (is_array($categories) && count($categories))
			foreach ($categories as $category_name)
			{
				...
				if (isset($category['id_category']) && $category['id_category'])
					$id_parent_category = $category['id_category'];
			}
		return $category;
	}
	...
	public function getParentsCategories($id_lang = null)
	{
		...
		if (count(Category::getCategoriesWithoutParent()) > 1 && Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && count(Shop::getShops(true, null, true)) != 1)
			$context->shop->id_category = Configuration::get('PS_ROOT_CATEGORY');
		...
		while (true)
		{
			...
			LEFT JOIN `'._DB_PREFIX_.'category_lang` cl
				...
				AND `id_lang` = '.$id_lang.Shop::addSqlRestrictionOnLang('cl').')';
			if (Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP)
				$sql .= ' LEFT JOIN `'._DB_PREFIX_.'category_shop` cs ON (c.`id_category` = cs.`id_category` AND cs.`id_shop` = '.$id_shop.')';
			$sql .= ' WHERE c.`id_category` = '.$id_current;
			if (Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP)
				$sql .= ' AND cs.`id_shop` = '.$context->shop->id;
			$root_category = Category::getRootCategory();
			if (Shop::isFeatureActive() && Shop::getContext() == Shop::CONTEXT_SHOP
				&& (!Tools::isSubmit('id_category') || Tools::getValue('id_category') == $root_category->id || $root_category->id == $context->shop->id_category))
	...
	*/
	...
	{
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'category_group` WHERE `id_category` = '.$this->id);
	}

	public function cleanAssoProducts()
	{
		Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'category_product` WHERE `id_category` = '.$this->id);
	}
	...
	public function addGroups($groups)
	{
		foreach ($groups as $group)
			if ($group !== false)
				Db::getInstance()->insert('category_group', array('id_category' => $this->id, 'id_group' => (int)$group));
	}

	public function getGroups()
	{
		$cache_id = 'Category::getGroups_'.$this->id;
		if (!Cache::isStored($cache_id))
		{
			$result = Db::getInstance()->executeS('
			SELECT cg.`id_group`
			FROM '._DB_PREFIX_.'category_group cg
			WHERE cg.`id_category` = '.$this->id);
			$groups = array();
	...
	public function addGroupsIfNoExist($id_group)
	{
		...
		if (!in_array($id_group, $groups))
			return $this->addGroups(array($id_group));
		return false;
	}
	...
	{
		$cache_id = 'Category::checkAccess_'.$this->id.'-'.$id_customer.(!$id_customer ? '-'.(int)Group::getCurrent()->id : '');
		if (!Cache::isStored($cache_id))
		{
			if (!$id_customer)
	...
	public static function setNewGroupForHome($id_group)
	{
		...
		VALUES ('.Context::getContext()->shop->getCategory().', '.(int)$id_group.')');
	}
	...
	public function updatePosition($way, $position)
	{
		if (!$res = Db::getInstance()->executeS('
			...
			WHERE cp.`id_parent` = '.$this->id_parent.'
			ORDER BY category_shop.`position` ASC'
		))
			return false;
		...
		foreach ($res as $category)
			if ($category['id_category'] == $this->id)
		...
		$result = (Db::getInstance()->execute('
			...
			'.($way
				...
				: '< '.(int)$moved_category['position'].' AND category_shop.`position` >= '.(int)$position).'
			AND c.`id_parent`='.$moved_category['id_parent'])
		&& Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'category` c '.Shop::addSqlAssociation('category', 'c').'
			SET category_shop.`position` = '.(int)$position.'
			WHERE c.`id_parent` = '.$moved_category['id_parent'].'
			AND c.`id_category`='.$moved_category['id_category']));
		Hook::exec('actionCategoryUpdate');
		return $result;
	}
	...
	public static function cleanPositions($id_category_parent = null)
	{
		...
		FROM `'._DB_PREFIX_.'category` c
		'.Shop::addSqlAssociation('category', 'c').'
		WHERE c.`id_parent` = '.$id_category_parent.'
		...
		for ($i = 0; $i < $count; $i++)
		{
			$return &= Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'category` c '.Shop::addSqlAssociation('category', 'c').'
			SET category_shop.`position` = '.($i).'
			WHERE c.`id_parent` = '.$id_category_parent.' AND c.`id_category` = '.$result[$i]['id_category']);
		}
		return $return;
	}
	...
	public static function getLastPosition($id_category_parent, $id_shop)
	{
		if (Db::getInstance()->getValue('
				...
				FROM `'._DB_PREFIX_.'category` c
				LEFT JOIN `'._DB_PREFIX_.'category_shop` cs ON (c.`id_category` = cs.`id_category` AND cs.`id_shop` = '.$id_shop.')
				WHERE c.`id_parent` = '.$id_category_parent) === 1)
			return 0;
		else
			return (1 + Db::getInstance()->getValue('
	...
	{
		$cache_id = 'Category::getInterval_'.$id;
		if (!Cache::isStored($cache_id))
		{
	...
	public static function inShopStatic($id_category, Shop $shop = null)
	{
		...
		if (!$interval = Category::getInterval($shop->getCategory()))
			return false;
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT nleft, nright FROM `'._DB_PREFIX_.'category` WHERE id_category = '.$id_category);
	...
	public function getDuplicatePosition()
	{
		...
		FROM `'._DB_PREFIX_.'category` c
		'.Shop::addSqlAssociation('category', 'c').'
		WHERE c.`id_parent` = '.$this->id_parent.'
		AND category_shop.`position` = '.$this->position.'
		AND c.`id_category` != '.$this->id);
	}
	...
	public function isParentCategoryAvailable($id_shop)
	{
		...
		AND c.`id_parent` = '.$this->id_parent);
	}
	...
	*/
	...
	{
		return Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'category_shop` WHERE `id_shop` = '.$id_shop);
	}
