...
abstract class ObjectModelCore
{
	...
	// Load object from database if object id is present
			$cache_id = 'objectmodel_'.$this->def['classname'].'_'.$id.'_'.(int)$this->id_shop.'_'.(int)$id_lang;
			if (!Cache::isStored($cache_id))
			{
				$sql = new DbQuery();
				$sql->from($this->def['table'], 'a');
				$sql->where('a.'.$this->def['primary'].' = '.$id);
	...
	{
		return $this->id > 0 ? $this->update($null_values) : $this->add($autodate, $null_values);
	}
		...
		if (!empty($this->def['multilang']))
		{
			...
			if ($fields && is_array($fields))
			{
				...
				foreach ($fields as $field)
				{
					...
					$field[$this->def['primary']] = $this->id;
	...
	public function duplicateObject()
	{
		...
		if (isset($definition['multilang']) && $definition['multilang'])
		{
			...
			{
				$row2[$definition['primary']] = $object_id;
		...
		$object_duplicated->duplicateShops($this->id);
	...
	public function update($null_values = false)
	{
		...
		if (Shop::checkIdShopDefault($this->def['table']) && !$this->id_shop_default)
			$this->id_shop_default = min($id_shop_list);
		// Database update
		if (!$result = ObjectModel::$db->update($this->def['table'], $this->getFields(), '`'.pSQL($this->def['primary']).'` = '.$this->id, 0, $null_values))
			return false;
		...
		if (Shop::isTableAssociated($this->def['table']))
		{
			...
			if (is_array($this->update_fields))
			{
				...
				$all_fields[$this->def['primary']] = $this->id;
			...
			foreach ($id_shop_list as $id_shop)
			{
				...
				$where = $this->def['primary'].' = '.$this->id.' AND id_shop = '.(int)$id_shop;
			...
			if (is_array($fields))
			{
				foreach ($fields as $field)
				{
					...
					if ($this->isLangMultishop())
					{
						...
						{
							$field['id_shop'] = (int)$id_shop;
							$where = pSQL($this->def['primary']).' = '.$this->id
	...
	public function delete()
	{
		...
		if (Shop::isTableAssociated($this->def['table']))
		{
			...
			$result &= ObjectModel::$db->delete($this->def['table'].'_shop', '`'.$this->def['primary'].'`='.$this->id.' AND id_shop IN ('.implode(', ', $id_shop_list).')');
		}

		// Database deletion
		$has_multishop_entries = $this->hasMultishopEntries();
		if ($result && !$has_multishop_entries)
			$result &= ObjectModel::$db->delete($this->def['table'], '`'.pSQL($this->def['primary']).'` = '.$this->id);
		...
		if (!empty($this->def['multilang']) && !$has_multishop_entries)
			$result &= ObjectModel::$db->delete($this->def['table'].'_lang', '`'.pSQL($this->def['primary']).'` = '.$this->id);
	...
	protected function makeTranslationFields(&$fields, &$fields_array, $id_language)
	{
		...
		$fields[$id_language][$this->def['primary']] = $this->id;
	...
	*/
	...
	public function isAssociatedToShop($id_shop = null)
	{
		...
		$cache_id = 'objectmodel_shop_'.$this->def['classname'].'_'.$this->id.'-'.(int)$id_shop;
		if (!Cache::isStored($cache_id))
		{
			$sql = 'SELECT id_shop
					FROM `'.pSQL(_DB_PREFIX_.$this->def['table']).'_shop`
					WHERE `'.$this->def['primary'].'` = '.$this->id.'
						AND id_shop = '.(int)$id_shop;
	...
	public function hasMultishopEntries()
	{
		if (!Shop::isTableAssociated($this->def['table']) || !Shop::isFeatureActive())
			return false; 
		return (bool)Db::getInstance()->getValue('SELECT COUNT(*) FROM `'._DB_PREFIX_.$this->def['table'].'_shop` WHERE `'.$this->def['primary'].'` = '.$this->id);
	}
